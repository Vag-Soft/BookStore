...
class BookControllerTest {
    ...
    void getBooksNoFilters() throws Exception {
        ...
        mockMvc.perform(get("/books").param("page", "0").param("size", "20").accept("application/json"))
                ...
                .andExpect(jsonPath("$.content[0].pages").value(1)).andExpect(jsonPath("$.content[0].price").value(1.0))
                .andExpect(jsonPath("$.content[0].availability").value(1))
                .andExpect(jsonPath("$.content[0].isbn").value("isbn"))
                .andExpect(jsonPath("$.content[0].genres").isEmpty()).andExpect(jsonPath("$.content[1].id").value(2))
                ...
                .andExpect(jsonPath("$.content[1].pages").value(2)).andExpect(jsonPath("$.content[1].price").value(2.0))
    ...
    void getBooksWithFilters() throws Exception {
        ...
        mockMvc.perform(get("/books").param("title", "title").param("genre", "genre").param("author", "author")
                .param("description", "desc").param("minPrice", "0.0").param("maxPrice", "100.0").param("page", "0")
                .param("size", "1").accept("application/json")).andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.pageable.pageSize").value(1)).andExpect(jsonPath("$.totalPages").value(2))
    ...
    void addBook() throws Exception {
        ...
        BookReadDTO savedBook = new BookReadDTO(3, "title3", "author3", "description3", 3, 3.0, 3, "isbn3",
                List.of(new GenreDTO(3, "genre1")));
        ...
        mockMvc.perform(post("/books").content(newBookString).contentType("application/json"))
                ...
                .andExpect(jsonPath("$.isbn").value(savedBook.getIsbn())).andExpect(jsonPath("$.genres").isArray())
                .andExpect(jsonPath("$.genres", hasSize(1))).andExpect(jsonPath("$.genres[0].id").value(3))
  ...
  void getBookByIDNotFound() throws Exception {
    ...
    void updateBookByIDFound() throws Exception {
        ...
        mockMvc.perform(put("/books/{bookID}", 1).contentType("application/json")
                .content(objectMapper.writeValueAsString(bookUpdateDTO))).andExpect(status().isOk())
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$.id").value(bookOutput.getId()))
                ...
                .andExpect(jsonPath("$.price").value(bookOutput.getPrice()))
                .andExpect(jsonPath("$.availability").value(bookOutput.getAvailability()))
                .andExpect(jsonPath("$.isbn").value(bookOutput.getIsbn())).andExpect(jsonPath("$.genres").isArray())
                .andExpect(jsonPath("$.genres", hasSize(1))).andExpect(jsonPath("$.genres[0].id").value(4))
